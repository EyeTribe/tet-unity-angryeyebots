...
function Update () {
	...
	if (state == false && joysticks[0].tapCount > 0) {
        ...
        /* @TheEyeTribe
         * 
         * Enabling 'fire' function joystick and ctrl buttons for Windows.
         */ 

        if (state == false && (Input.GetMouseButtonDown (0) || Input.GetKeyDown ("joystick button 0") || Input.GetKeyDown ("left ctrl")) )
		{
			mouseDownSignals.SendSignals (this);
			state = true;
		}
        else
        if (state == true && (Input.GetMouseButtonUp (0) || Input.GetKeyUp ("joystick button 0") || Input.GetKeyUp ("left ctrl")) ) 
		{
			mouseUpSignals.SendSignals (this);
			state = false;
		}
